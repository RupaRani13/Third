<template>
    <div id="fromDesign05">
        <template v-if="type == 'text'">
            ...
            <v-text-field validate-on="input" v-else :label="label" v-model="modelValue" variant="outlined"
        ...
        <template v-if="type == 'email'">
            ...
            <v-text-field validate-on="input" v-else :label="label" v-model="modelValue" variant="outlined"
                @input="$emit('update:modelValue', $event.target.value)" :rules="modelValue ? [rules.email] : ''">
            </v-text-field>
        </template>
        ...
        <template v-if="type == 'textarea'">
            <v-textarea validate-on="input" v-if="required" :label="`${label}*`" v-model="modelValue" variant="outlined"
                autoGrow shaped rows="4" row-height="50" @input="$emit('update:modelValue', $event.target.value)"
                :rules="[rules.required]">
            </v-textarea>
            <v-textarea validate-on="input" v-else :label="label" v-model="modelValue" variant="outlined" autoGrow
                shaped rows="4" row-height="50" @input="$emit('update:modelValue', $event.target.value)" :rules="''">
            </v-textarea>
        </template>
        <template v-if="type == 'dropdown'">
            <client-only>
                <v-select :items="options" :label="required ? `${label}*` : label" v-model="modelValue"
                    variant="outlined" :clearable="!required"
...
<style>
#fromDesign05 .checkboxmsg {

    position: relative;
    flex-wrap: wrap;
}

#fromDesign05 .checkboxmsg .v-input.v-input--horizontal.v-input--density-default.v-checkbox {
    max-width: 130px;
    width: 100px;
}

#fromDesign05 .checkboxmsg .v-input {
    flex: none;
}

#fromDesign05 .checkboxmsg .v-checkbox .v-input__details .v-messages {
    display: none;

}

#fromDesign05 .checkboxmsg .v-checkbox:last-child .v-input__details .v-messages {
    display: block;
}

#fromDesign05 .checkboxmsg .v-checkbox .v-input__details {
    position: absolute;
    top: 0;
    bottom: 0;
    left: 0;
