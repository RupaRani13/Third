    ...
    <div id="fromDesign03">      
        <template v-if="type == 'text'" >
                <v-text-field validate-on="input" v-if="required" :label="`${label}*`"
                    v-model="modelValue"   
                    variant="underlined" @input="$emit('update:modelValue', $event.target.value)" :rules="[rules.required, rules.specialcharacter]">
                </v-text-field>
                <v-text-field validate-on="input" v-else :label="label"
                    v-model="modelValue"  
                    variant="underlined"  @input="$emit('update:modelValue', $event.target.value)" :rules="[rules.specialcharacter]">
                </v-text-field>
        </template> 
 
          <template v-if="type == 'mobile'">
                <v-text-field validate-on="input" v-if="required" :label="`${label}*`"
                    v-model="modelValue"   variant="underlined" @input="$emit('update:modelValue', $event.target.value)" :rules="modelValue ? [rules.mobileNumber] : [rules.required]"  >
                </v-text-field>
                <v-text-field validate-on="input" v-else :label="label"
                    v-model="modelValue"  variant="underlined"  @input="$emit('update:modelValue', $event.target.value)" :rules="modelValue ? [rules.mobileNumber] : ''"  >
                </v-text-field>
        </template>
       <template v-if="type == 'email'">
                <v-text-field validate-on="input" v-if="required" :label="`${label}*`"
                    v-model="modelValue"  variant="underlined"  @input="$emit('update:modelValue', $event.target.value)" :rules="modelValue ? [rules.email] : [rules.required]"  >
                </v-text-field>
                <v-text-field validate-on="input" v-else :label="label"
                    v-model="modelValue"  variant="underlined"  @input="$emit('update:modelValue', $event.target.value)" :rules="modelValue ? [rules.email] : ''" >
                </v-text-field>
        </template>
      <template v-if="type == 'number'">
                <v-text-field validate-on="input" v-if="required" :label="`${label}*`"
                    v-model="modelValue"  variant="underlined"  @input="$emit('update:modelValue', $event.target.value)" :rules="[rules.required, rules.number]" >
                </v-text-field>
                <v-text-field validate-on="input" v-else :label="label"
                    v-model="modelValue"  variant="underlined"  @input="$emit('update:modelValue', $event.target.value)" :rules="''" >
                </v-text-field>
        </template>
      <template v-if="type == 'textarea'">
                <v-textarea validate-on="input" v-if="required" :label="`${label}*`"
                    v-model="modelValue"  variant="underlined" autoGrow shaped rows="4" row-height="50" @input="$emit('update:modelValue', $event.target.value)" :rules="[rules.required]" >
                </v-textarea>
                <v-textarea validate-on="input" v-else :label="label"
                    v-model="modelValue"  variant="underlined" autoGrow shaped rows="4" row-height="50" @input="$emit('update:modelValue', $event.target.value)" :rules="''">
                </v-textarea>
        </template> 
        <template v-if="type == 'dropdown'">
            <client-only>
            <v-select :items="options" :label="required ? `${label}*` : label" 
                v-model="modelValue"  variant="underlined" :clearable="!required"  @update:modelValue="$emit('update:modelValue', modelValue)" :rules="required?[rules.required]:''">
            </v-select>
            </client-only>
        </template> 
 <template v-if="type == 'radio'">
            <div style="text-transform: capitalize;">
                ...
                <v-radio-group inline v-model="modelValue" @change="$emit('update:modelValue', modelValue)"  :label="required?`${label}*`:label" :rules="required?[rules.required]:''">
                    <v-radio v-for="item in options" :key="item" :label="item" :value="item" color="success"  variant="underlined">
                    </v-radio>
                </v-radio-group>
                </client-only>
            </div>
  ...
  <template v-if="type == 'checkbox'">
        ...
        <template v-if="type == 'file'">
                <v-file-input v-if="required" :label="`${label}*`"
                    v-model="bindingValue" variant="underlined" clearable rows="4" :show-size="1024" accept="image/png, image/jpeg, image/bmp" row-height="30" @update:modelValue="onFileSelected()" :rules="[rules.fileRequired] " >
                </v-file-input>
                <v-file-input v-else :label="label"
                    v-model="bindingValue" variant="underlined" clearable  rows="4" :show-size="1024" accept="image/png, image/jpeg, image/bmp" row-height="30" @update:modelValue="onFileSelected()" :rules="[rules.fileSize]">
                </v-file-input>
        </template> 
    </div>
